...
\begin{frame}{Da função matemática ao programa}
  ...
  \begin{equation}\label{eq:raiz-dominio}
    f:\mathbb{R}_{\geq 0}\longrightarrow \mathbb{R}_{\geq 0},\qquad
    f(x)=\sqrt{x}.
  \end{equation}
...
\begin{frame}{Funções que recebem funções}
  ...
  Se $g(x)=x+1$:
  \begin{equation*}
    (\lambda g.\lambda x.\;g(g(x)))\;(\lambda y.\;y+1)\;3
    \longrightarrow
    5.
  \end{equation*}
...
\begin{frame}{Síntese}
  \begin{enumerate}
    ...
    \item Métodos numéricos podem ser vistos como composição e aplicação repetida de funções.
  \end{enumerate}

  \vspace{0.2cm}
  \begin{block}{Mensagem final}
    A programação funcional não substitui os métodos numéricos: ela oferece uma forma mais explícita de estruturá-los.
  \end{block}
\end{frame}
